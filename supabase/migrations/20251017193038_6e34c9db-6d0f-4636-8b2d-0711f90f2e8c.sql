-- Add status field to assessments table (only if it doesn't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE assessments ADD COLUMN status assessment_status NOT NULL DEFAULT 'pending';
  END IF;
END $$;

-- Add evidence field to assessment_results (only if it doesn't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'assessment_results' AND column_name = 'evidence'
  ) THEN
    ALTER TABLE assessment_results ADD COLUMN evidence text;
  END IF;
END $$;
