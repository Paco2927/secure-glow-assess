...
-- Recreate the view with SECURITY INVOKER (uses the permissions of the querying user)
...
SELECT 
  ...
  CASE 
    ...
    WHEN public.has_role(auth.uid(), 'admin'::app_role) OR public.has_role(auth.uid(), 'moderator'::app_role) THEN 'admin'
